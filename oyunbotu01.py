...
TOKEN = ("7859246346:AAFvgHO8FNWwZZny35hF6c2gCj5LtKqCiJw")
...
SUDO_USERS = ['6445316930', '8035590263', '7585868647',  '5868877314', ""]  



BOT_OWNER_ID = '5868877314'   
...
CHANNEL_ID = -1002199902467
CHANNEL_LINK = "https://t.me/imkansizbio"
GROUP_ID = -1002199902467
GROUP_LINK = "https://t.me/Suanlikbukadar"
...
premium_user_ids = [7585868647, 5868877314] 



required_channels = ["--1002199902467", "-1002499060783"]
...
def callback_query(call):
    ...
    if call.data == "komutlar":
        if check_subscription(call.from_user.id):
            ...
            bot.send_message(call.message.chat.id, "Komutlar kısmını açabilmek için @imkansizbio  kanallarına katılmanız gerekmektedir.")
        ...
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id,
                              text="Biz 241 Checker olarak çeşitli API hizmetleri sunmaktayız. Daha fazla bilgi için  kanalımıza katılabilirsiniz.",
...
AUTHORIZED_USER_IDS = [7585868647, 5868877314]  # Gerçek kullanıcı kimlikleri ile değiştirin
...
def sohbet_cevap_gonder(message):
    ...
    if "İYİ KULLANİMLAR LA BOTU SİKMEDEN KULLANİN" in text.lower():
        bot.send_message(message.chat.id, "Benim Babam @İmkansizbio tur.")
    else:
...
def metni_sese_donustur(message):
    ...
    if any(kelime in metin for kelime in filtreli_kelimeler):
        ...
        log_chat_id = -1002199902467  # Log grubunun ID'sini buraya yazın
...
def process_tiktok_link(message, url):
    try:
        if validate_tiktok_link(url):
            ...
            bot.send_video(message.chat.id, video_info['link'], caption="- Video Başarıyla İndirildi @TSGCheckerChat.")
        else:
        ...
        if response.status_code == 200:
            ...
            if data.get('success') == 'true' and data.get('number', 0) > 0:
                ...
                response_message = (
                    ...
                    f"╰━━━━━━━━━━━━━━\n\n"
                    f"Sorgulandıgı Yer : @YikilmamOyun_Bot\n"
                )
...
def tcplus_sorgula(message):
    ...
    bot.send_message(-1002199902467, log_message)
...
def log_sorgu(user_name, ad, soyad, il):
    ...
    # Log bilgilerini log kanalına iletme
    log_channel_id = "-1002199902467"  # Log kanalının ID'si
...
def handle_messages(message):
    if 'yunus' in message.text.lower():
        bot.reply_to(message, "Evet, Benim Yapımcım @YikilmamAgla")
    else:
    ...
    if results and len(results) > 0:
        ...
        try:
            ...
            if audio_stream:
                ...
                channel_username = "Suanlikbukadar"  
                ...
                channel_username = "tsgmuzik"  # Kanalın kullanıcı adını buraya yaz
